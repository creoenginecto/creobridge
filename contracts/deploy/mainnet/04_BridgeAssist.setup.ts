...
const func = setup('BridgeAssist', async () => {
  ...
  const [, txr] = await safeWrite(
    bridgeAssist.connect(deployer).grantRole(MANAGER_ROLE, deployer.address)
  )

  const [, txr3] = await safeWrite(
    bridgeAssist.connect(deployer).addChains(['evm.56', 'evm.1'], [0, 0])
  )
